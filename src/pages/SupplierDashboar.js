...
const SupplierDashboard = () => {
  const { user, getAuthHeaders, loading, logout } = useAuth(); // ✅ FIXED: added logout
...
useEffect(() => {
  console.log("User from AuthContext:", user); // Add this
  if (!loading && user?._id) {
    fetchProducts();
    fetchBulkOrders();
  }
 ...
 const fetchProducts = async () => {
  ...
  if (!user?._id) return; // ✅ safety check
  try {
    const res = await axios.get(`${BULK_API}/${user._id}`, getAuthHeaders());
    console.log('Bulk Orders API response:', res.data);
  ...
  const handleCreateBulkOrder = async () => {
    ...
    try {
      if (isEditBulkMode && editBulkOrderId) {
        await axios.put(
          `${BULK_API}/${editBulkOrderId}`,
          {
            productName: bulkOrder.productName,
            quantity: Number(bulkOrder.quantity),
            price: Number(bulkOrder.price),
            supplierId: user._id,
            supplierName: user.business_name,
